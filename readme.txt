== Compilation ==

Contributors: Naeem Noor
Requires at least: 6.0
Tested up to: 6.6
Requires PHP: 5.7
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html


== Description ==

A minimalist block theme for content curators. With its clean and minimal design, the Compilation WordPress theme enables users to easily build curated lists and directory websites with WordPress.

== Changelog ==

= 1.0.7 =
* Fixed responsive menu color scheme
* Fixed style.css path

= 1.0.6 =
* Screenshot updated
* Added 4 new patterns
* Fixed no-title template full width alignment
* Fixed input:focus background color contrast

= 1.0.5 =
* Added new style variation
* Restyled submenu
* Restyled mobile menu
* Categorized patterns
* Added recent posts inside single
* Moved basic styling to assets/css/style.css
* Restyled comments
* Fixed pagination css

= 1.0.4 =
* Updated theme.json
* Removed unnecessary underline from links
* Updated screenshot

= 1.0.3 =
* Fixed content alignment for archive template
* Fixed content alignment for page template

= 1.0.2 =
* Removed default gradient
* Removed default color palette
* Updated footer credits
* Changed base color to #FFF
* Fixed screenshot alignment
* contentSize & wideSize updated
* Added underline on hover for navigation links

= 1.0.1 =
* Added underline for links
* Updated theme URI and Author URI
* Fixed screenshot
* Added font licenses

= 1.0.0 =
* Initial release


== Copyright ==

Compilation WordPress Theme, (C) 2024 Naeem Noor
Compilation is distributed under the terms of the GNU GPL.

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 2 of the License, or
(at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU General Public License for more details.


== Fonts ==

IBM Plex Sans
Copyright 2018 IBM Corp. All rights reserved.
Source: http://www.boldmonday.com
License: This Font Software is licensed under the SIL Open Font License, Version 1.1. This license is available with a FAQ at: http://scripts.sil.org/OFL

